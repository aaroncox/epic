 ...
 **/
class Epic_View_Helper_Card extends Epic_View_Helper_HtmlTag
{
	// public function profileCard(Epic_Mongo_Profile $profile) {
	// 	return
	// }
	public function link($record, $params = array()) {
		if ($record->id) {
			if($record instanceOf Epic_Mongo_Profile) {
			...
			return $this->view->mLink($record, $params);
		} else {
	...
	public function cardScore($record) {
		if(!$record instanceOf Epic_Mongo_Profile_User) return '';
	...
	public function card($record, $params = array()) {
		...
		if(isset($params['tagType'])) {
			$this->setTagType($params['tagType']);
		}
		if(!$record) {
			return null;
			// $record = Epic_Mongo::newDoc('user');
		...
		if(!$record instanceOf Epic_Interface_Cardable) return '';
