 ...
 * @license     http://hobodave.com/license.txt New BSD License
 **/
class Epic_View_Helper_BundleLink extends Zend_View_Helper_HeadLink
{
	/**
	 ...
	 * @var string
	 */
	protected $_regKey = 'Epic_View_Helper_BundleLink';
	...
	public function toString($indent = null)
	{
		...
		// no files to deal with, don't create anything
		if (!$filelist) {
			return "";
		}
		$hash = md5($filelist);
		...
		if ($this->_doMinify) {
			if (!empty($this->_minifyCallback)) {
				...
				file_put_contents($cacheFile, $data);
							...
							file_put_contents("$cacheFile.uncompressed", $data);
			  $command = str_replace(':filename', $cacheFile.".uncompressed", $this->_minifyCommand);
			  $handle = popen("$command" , 'r');
							file_put_contents($cacheFile, $handle);
			pclose($handle);
			} else {
