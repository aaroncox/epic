<?php
class Epic_Filter_TagJSON implements Zend_Filter_Interface {
	protected $_type;
	protected $_limit;
	...
	public function toArray($value)
	{
		...
		foreach ( $refs as $value ) {
			if ( isset( $value['new'] ) && Epic_Auth::getInstance()->hasPrivilege( new Epic_Auth_Resource_Tag, "create" ) ) {

				$ref = Epic_Mongo::newDoc( $value['type'] );
				$ref->name = $value['name'];
				$ref->tags->setTag('author', Epic_Auth::getInstance()->getUserProfile());
				$ref->_created = time();
				$ref->save();
				$return[] = $ref;
