...
class Epic_Auth_Adapter_MongoDb implements Zend_Auth_Adapter_Interface
{
    ...
    protected function _setCollection(MongoCollection $collection = null)
    {
        ...
        if(null === $this->_collection) {
            $this->_collection = Epic_Mongo::db('user');
            if (null === $this->_collection) {
    ...
    public function authenticate()
    {
        ...
        $identity = $this->_collection->fetchOne(array(
            $this->_identityKeyPath => $this->_identity
        ));
        if($identity == null) {
            $this->_authenticateResultInfo['code'] = Zend_Auth_Result::FAILURE_IDENTITY_NOT_FOUND;
            $this->_authenticateResultInfo['messages'][] = 'A record with the supplied identity could not be found.';
        } elseif ($identity instanceOf Epic_Mongo_Document_User) {
            $this->_resultDoc = $identity;
            if ($identity[$this->_credentialKeyPath] == md5($this->_credential)) {
								$this->_authenticateResultInfo['identity']['id'] = (string) $identity->_id;
